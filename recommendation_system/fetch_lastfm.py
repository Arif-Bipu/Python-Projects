...
def no_file():
    ...
    print ('then place it in recommendation_system/data/ and change the file_path')
    print ('variable on the fetch_lastfm function')


def fetch_lastfm(min_plays=200):
   
    file_path = 'data/100klines.tsv'
    ...
    coo = coo_matrix((data,(row,col)), shape=(len(artists), len(users)))
